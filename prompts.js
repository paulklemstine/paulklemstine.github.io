...
//     - The hidden analysis elements are no longer generated by this prompt.

// 4.  **Flavor Element (divine_wisdom):**
//     - You SHOULD include one specific text element for flavor.
//     - **`divine_wisdom`**: A text element (`type: "text"`, `name: "divine_wisdom"`) with a short, cryptic, meta-commentary from a "God" voice. This should be a single, profound or funny sentence.

// 5.  **CRITICAL UI ELEMENT RULES:**
...
//     - Its \`value\` MUST be a stringified JSON object, based on the full, structured notes object provided in the turn-specific instructions.
//     - You MUST pass this JSON object verbatim. Do not alter its structure.
...
// - **Update State (CRITICAL):** You MUST update the structured JSON 'notes' object for each player. This is your primary task.
//     - **`subjectId`, `playerName`, `physicalDescription`**: Update with any new demographic info from the player's input.
//     - **`phase` & `phase_turn`**: Advance the turn counter. Decide if the game should move from 'Assessment' to 'Guidance' phase, etc.
//     - **`immediate_goals`, `mid_term_goals`, `long_term_goals`**: Update based on player actions. What is the next logical step for their "treatment"?
//     - **`strategies`**: What psychological techniques should be used next?
//     - **`behavioral_patterns` & `emotional_trends`**: Note recurring choices, emotional responses, etc.
//     - **`deviance_flags`**: Add any new observations of strange or deviant behavior.
...
//     - **`diagnostic_axes`**: Update the multi-axial assessment. Based on player choices, add or update diagnoses, confidence levels, and supporting notes for any of the 16 axes. Focus on deviancy (axes 2, 11, 13, 15, 16).
//     - **`overriding_game_changes`**: Modify the game's visual style, pacing, or difficulty if necessary.
